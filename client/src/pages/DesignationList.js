...
class DesignationList extends Component {
  ...
  confirmDialogDelete(id) {
    this.props.actionsDesignation.deleteDesignation(this.state.idDelete).then(data => {
      this.props.actionsDesignation.loadDesignationList();
      this.setState({ openDialogDelete: false, idDelete: null });
    });
  }
            ...
            {this.props.list.map(row => (
              <TableRow key={row._id}>
                <TableCell component="th" scope="row">
                  <Link to={"/designations/" + row._id} key={row._id}>
                    {row._id}
                  </Link>
                </TableCell>
...
function mapStateToProps(state, ownProps) {
  return {
    list: state.DesignationListReducer.listDesignation
  };
}
